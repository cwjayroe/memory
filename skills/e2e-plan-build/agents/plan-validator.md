...
- `plan_key`: Memory upsert key for the top-level plan.
- `plan_phase_key_template`: Template for per-phase keys.
- `architecture_snapshot_key`: Memory upsert key for the architecture snapshot.
- `phase_count`: Total number of phases.

## Protocol

### 1. Retrieve plan artifacts

Use `get_memory` to retrieve:
- The top-level plan (from `plan_key`).
- Each phase spec (from `plan_phase_key_template` with `{N}` = 1 to `phase_count`).
- The architecture snapshot (from `architecture_snapshot_key`).
...
Store the report in memory at `{repo}::e2e-build::{build_id}::plan-validation`:
